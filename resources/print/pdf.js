...
const decorList = ({doc, setting, options}) => {
    // Натройка ширифта
    doc.setFont('museo');
...
// Вывод размеров
const sheetMeshSetting = ({doc, tables}) => {
    // Параметры листа
...
const pdfRender = async (setting, data, language) => {
    ...
    if (!isMultiplication) {
        ...
        let count_page = renderTableOne({
            doc,
            rows: data.data.map((task) => task.exercise),
            settings: setting,
            startPage: null,
            title,
        });
        ...
        renderTableOne({
            doc,
            rows: data.data.map((task) => task.answer),
            settings: setting,
    ...
    const path = `public/print/lists/Листы тренировка ID ${data.id}.pdf`
    doc.save(path)
    return path
};
...
//
const updateTheRowsInTheTables = ({_rows, settings, columns, answ}) => {
    let alpha = alphabet('A', 'Z'),
        tableForPdf = [],
        tables = chunk(_rows, settings.column);
...
const updateTheRowsInTheTablesTwo = ({_rows, settings, columns, answ}) => {
    let alpha = alphabet('A', 'Z'),
        tableForPdf = [],
        tables = chunk(chunk(_rows, settings.column), settings.rows);

    tables.map((table, k) => {
        tableForPdf[k] = [];
        return table.map((rows, ke) => {
            let answerEmpty = [],
                columnEmpty = [];
...
module.exports = {pdfRender, alphabet}